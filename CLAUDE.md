...
- **Fonts:** EB Garamond serif and DM Sans sans (Google Fonts, one request), Commit Mono (Fontsource via jsDelivr) — all loaded via `_includes/head.html`. The navbar name uses Movement Direct Thin, self-hosted from `assets/fonts/` via `@font-face` in `styles.css`.
