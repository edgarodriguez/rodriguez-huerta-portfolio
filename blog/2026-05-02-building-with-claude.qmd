...
flowchart TD
    A([Claude design<br>Chat]) --> B([Claude generates<br>HTML prototype])
    B --> C([Extract design tokens<br>to styles.css])
    C --> D([Build Quarto structure<br>_quarto.yml + layouts])
    D --> E([Write content<br>as .qmd files])
    E --> F([quarto render → _site/])
    F --> G{Browser DevTools<br>inspect}
    G -->|Wrong selector<br>or layout bug| H([Claude Code<br>fix CSS])
    G -->|New interaction<br>or animation| I([Claude Code<br>implement JS/GSAP])
    H --> F
    I --> F
    G -->|Looks right| J([git push<br>GitHub Pages])
